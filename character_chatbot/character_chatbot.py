...
class CharacterChatBot():
    ...
    def __init__(self ,
                 model_path ,
                 data_path = "/contnt/data/naruto.csv",
                 huggingface_token = None,
                 ):
        
        self.model_path = model_path 
        self.data_path = data_path
        self.huugingface_token = huggingface_token
        self.base_model_path = "meta-llama/Llama-3.1-8B-Instruct"
        self.device = "cuda" if torch.cuda.is_available() else 'cpu' 
        
        
        if self.huugingface_token is not None :
            huggingface_hub.login(self.huugingface_token)
            
        if huggingface_hub.repo_exists(self.model_path):
            self.model = self.load_model(self.model_path)
        else :
            print("model does not exist in huggingface hub we will train our own model ")
    ...
    def load_data(self):
        ...
        indexes_to_take = list(naruto_transcript_df[(naruto_transcript_df['naruto_responce_flag']==1)&(naruto_transcript_df.index>0)].index)
