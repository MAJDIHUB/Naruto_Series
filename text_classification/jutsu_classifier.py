...
class JutsuClassifier():
    def __init__( self , 
        ...
        if not  huggingface_hub.repo_exists(self.model_path):
            ...
            if data_path is None :
                raise ValueError("data path is required to train the model , since the model path does not exist in hugginface hub  ")
    ...
    def load_data (self , data_path):
        ...
        df['jutsu_type_simplifeid'] = df['jutsu_type'].apply(self.simplify_jutsu)
        df['text'] = df['jutsu_name'] + " ." + df['jutsu_description']
        df[self.label_column_name] = df['jutsu_type_simplifeid']
